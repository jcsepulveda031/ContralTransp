...
const EditAlmacenModal: React.FC<EditAlmacenModalProps> = ({
  ...
  const [formData, setFormData] = useState<Almacen>(almacen);
  ...
  useEffect(() => {
    if (isOpen) {
      // Encontrar el departamento basado en la ciudad actual
      const depObj = departamentosCiudades.find(dep => 
        dep.ciudades.includes(almacen.ciudad)
      );
      if (depObj) {
        setDepartamento(depObj.nombre);
        setCiudades(depObj.ciudades);
      }
      setFormData(almacen);
      setError('');
    }
